...
theorem add_left_cancel {a b c : R} (h : a + b = a + c) : b = c := by
  rw [ ← add_neg_cancel_right b a, add_comm b, h, add_comm a, add_assoc, add_comm a, neg_add_cancel, add_zero]
...
theorem mul_inv_cancel (a : G) : a * a⁻¹ = 1 := by
  ...
  rw [mul_assoc, mul_assoc, ← mul_assoc a⁻¹, inv_mul_cancel, one_mul, inv_mul_cancel]

theorem mul_one (a : G) : a * 1 = a := by
  rw [← inv_mul_cancel a, ← mul_assoc, mul_inv_cancel, one_mul]

theorem mul_inv_rev (a b : G) : (a * b)⁻¹ = b⁻¹ * a⁻¹ := by
  rw [← one_mul b⁻¹, ← inv_mul_cancel (a * b), mul_assoc, mul_assoc, mul_assoc, ←  mul_assoc b b⁻¹, mul_inv_cancel, one_mul, mul_inv_cancel, mul_one]
